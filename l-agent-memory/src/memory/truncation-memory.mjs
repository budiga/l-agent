...
async function messageCountTruncation() {
  ...
  let allMessages = await history.getMessages() // 按消息数量截断：保留最近 maxMessages 条消息
...
async function tokenCountTruncation() {
  ...
  let allMessages = await history.getMessages() // 使用 trimMessages API：使用 js-tiktoken 计算 token 数量
